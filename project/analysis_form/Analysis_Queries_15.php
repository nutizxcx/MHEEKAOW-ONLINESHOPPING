...
$con=openCon();
$result = mysqli_query($con,"SELECT p.ProductID, COUNT(p.ProductID)*(p.SellingPrice-p.Cost) AS 'Profit'
FROM product_infomation p JOIN order_product op ON p.ProductID = op.ProductID JOIN order_history oh ON Oh.OrderID = op.OrderID
...
<center>
<br>
<h1>This report shows the profit receive from each product in each month.<span id="name"></span></h1><br>
Time:2019-04-24 To 2019-05-24<span id="name2"></span>
<table id="customers">
    <thead>
      <tr>
        <th>ProductID</th>
