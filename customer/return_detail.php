...
$result = mysqli_query($con,"SELECT o.OrderID,o.ProductID ,o.SKU,p.Name,s.Size,s.Color,s.Group,s.Category
                             FROM order_product o JOIN product_infomation p ON o.ProductID = p.ProductID
                                                  JOIN skudetail s ON o.SKU = s.SKU 
                             WHERE o.OrderID = $orderid ORDER BY ProductID ");
...
<?php
  echo "<h3 style=' position:absolute; left:24.5%; '>Please select the product ID that you want to return.</h3>";
?>

    <form action="return_sku.php" method="post" name="select_productID">
      <br><br>
    <?php echo "<p style='font-size:15px; position:absolute; left:24.5%;'>Select ProductID: </p>" ?>
    <br><br>
          <select name="productID_select" style='font-size:15px; position:absolute; left:24.5%;'>
          
            <option value="0"><-- Please Select --></option> <br>
            <?php
            $objQuery = mysqli_query($con,"SELECT DISTINCT o.ProductID 
                                           FROM order_product o JOIN product_infomation p ON o.ProductID = p.ProductID 
                                                                JOIN skudetail s ON o.SKU = s.SKU WHERE o.OrderID = $orderid 
                                           ORDER BY ProductID ");
            ...
            ?>
            <input type="hidden" name="orderid" value="<?php echo $orderid  ?>">  <br><br><br>
            <input class='returnChooseBut' name="btnSubmit" type="submit" value="Submit" style='font-size:15px; position:absolute; left:24.5%; '>
    </form>
